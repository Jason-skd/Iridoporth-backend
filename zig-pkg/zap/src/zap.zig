...
fn sleepFor(nanoseconds: i96) !void {
    var threaded: std.Io.Threaded = .init(std.heap.smp_allocator, .{});
    defer threaded.deinit();
    try threaded.io().sleep(.fromNanoseconds(nanoseconds), .awake);
}
...
pub const HttpListener = struct {
    ...
    pub fn listen(self: *HttpListener) !void {
        ...
        try sleepFor(500 * std.time.ns_per_ms);

        var portbuf: [100]u8 = undefined;
        const printed_port = try std.fmt.bufPrintSentinel(&portbuf, "{d}", .{self.settings.port}, 0);
