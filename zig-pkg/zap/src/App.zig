...
pub fn Create(
    ...
    return struct {
        ...
        pub fn deinit() void {
            ...
            const io = _static.io_threaded.io();
            _static.track_arena_lock.lockUncancelable(io);
            defer _static.track_arena_lock.unlock(io);
        ...
        pub fn get_arena() !*ArenaAllocator {
            ...
            const io = _static.io_threaded.io();
            _static.track_arena_lock.lockSharedUncancelable(io);
            if (_static.track_arenas.getPtr(thread_id)) |arena| {
                _static.track_arena_lock.unlockShared(io);
                return arena;
            } else {
                _static.track_arena_lock.unlockShared(io);
                _static.track_arena_lock.lockUncancelable(io);
